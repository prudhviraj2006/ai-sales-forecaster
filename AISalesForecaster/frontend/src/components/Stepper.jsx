...
function Stepper({ currentStep, onStepClick, hasUploadData, hasForecastData, onReset }) {
  ...
  return (
    <>
      <div className="hidden md:flex fixed left-0 top-0 h-screen w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 border-r border-slate-700 flex-col z-40 shadow-2xl">
      ...
      <div className="md:hidden fixed bottom-0 inset-x-0 bg-slate-900/95 border-t border-slate-700 backdrop-blur-sm z-40">
          ...
          {menuItems.slice(0, 4).map((item) => {
            ...
            return (
              <button
                ...
                onClick={() => handleMenuClick(item.id)}
                disabled={isDisabled}
                className={`flex flex-col items-center justify-center gap-1 rounded-lg py-2 text-xs ${
                  isActive ? 'bg-blue-600 text-white' : isDisabled ? 'text-slate-600' : 'text-slate-300'
                }`}
              >
                <Icon size={16} />
        ...
        {currentStep !== 'upload' && (
          <div className="px-3 pb-3">
            <button
              onClick={onReset}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-amber-600 text-white rounded-lg font-medium"
            >
              <RefreshCw size={16} />
              <span>Start Over</span>
            </button>
